...
The `image_generate` tool uses the laptop GPU ComfyUI service through the
`comfyui-local` Hermes provider plugin. It submits the same SDXL workflow
...
text-to-image; editing and reference images are not supported. Requests time
out after ten minutes if the laptop is offline or generation takes too long.
...
and the laptop's `journalctl -u comfyui.service -f` if either fails. Test
SearXNG directly with
`podman exec hermes curl 'http://searxng:8080/search?q=test&format=json'`.
